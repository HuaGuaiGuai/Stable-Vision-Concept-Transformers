...
def save_target_activations(target_model, dataset, save_name, target_layers = ["layer4"], batch_size = 16,
    ...
    with torch.no_grad():
        for images, labels in tqdm(DataLoader(dataset, batch_size, num_workers=0, pin_memory=False)):
...
def save_clip_image_features(model, dataset, save_name, batch_size=1000 , device = "cuda"):
    ...
    with torch.no_grad():
        for images, labels in tqdm(DataLoader(dataset, batch_size, num_workers=0, pin_memory=True)):
...
def save_activations(clip_name, target_name, target_layers, d_probe, 
                     concept_set, batch_size, device, pool_mode, save_dir, dataset, num_classes):
    ...
    if target_name.startswith("clip_"):
        ...
        target_model, target_preprocess = data_utils.get_target_model(target_name, device, dataset, num_classes)
...
def get_accuracy_cbm(model, dataset, device, batch_size=16, num_workers=0, dataset_name = ''):
    correct = 0
    total = 0
    prob_all = []
    label_all = []
    ...
    for images, labels in tqdm(DataLoader(dataset, batch_size, num_workers=num_workers,
        ...
        with torch.no_grad():
            ...
            prob_all.extend(outs[:,1].cpu().numpy()) 
...
#             print("outputs shape:{}".format(outs.shape))

            outputs2 = outs.argmax(dim=1)
#             print("outputs2 shape:{}".format(outputs2.shape))

            true_label_list.append(labels.cpu().detach().numpy())
            pred_label_list.append(outputs2.cpu().detach().numpy())
...
def get_concept_act_by_pred(model, dataset, device):
    preds = []
    concept_acts = []
    for images, labels in tqdm(DataLoader(dataset, 500, num_workers=0, pin_memory=True)):
        with torch.no_grad():
